...
def planner_node(state: PevState) -> Dict:
    ...
    # Include Global Wisdom (persistent expertise from previous trials)
    wisdom_section = ""
    if state.global_wisdom:
        wisdom_section = "\nGLOBAL EXPERTISE (Lessons learned from previous technical failures — MANDATORY COMPLIANCE):\n"
        for i, w in enumerate(state.global_wisdom[-10:]): # Last 10 insights
            wisdom_section += f"- {w}\n"
