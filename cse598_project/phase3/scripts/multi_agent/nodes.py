...
9. ID AND CODE LOOKUP: Before using any parameter expecting an ID or code (item ID, flight number, airport IATA code, reservation ID, product SKU), retrieve it from the API or wiki first. NEVER pass city names into airport fields — use IATA codes. NEVER guess or invent IDs.
...
13. READ ALL SEARCH RESULTS: After any search or list tool, read ALL returned entries before concluding no options exist. Do NOT say "no options available" unless the API returned an empty list. If results exist, filter by user preferences and pick the best match.
