...
def format_memory(memory_list: List[Dict], max_steps: int = 15) -> str:
    """Converts the raw JSON memory array into a clean structural markdown trace, 
    truncating massive observations (like flight searches) to save context window tokens."""
    if not memory_list:
        return "No prior history."
    
    # Only keep the most recent N steps to prevent window overflow
    trunc_list = memory_list[-max_steps:]
    
    out = []
    for i, m in enumerate(trunc_list):
        step_idx = len(memory_list) - len(trunc_list) + i + 1
        
        obs = str(m.get('api_observation', 'None'))
        # TRUNCATION LOGIC: Massive search results or long profile data can crash the 30k window.
        # We keep the first 1500 chars which usually contains the critical IDs/prices.
        if len(obs) > 1500:
            obs = obs[:1500] + "... [TRUNCATED TO SAVE CONTEXT]"
        ...
        if m.get('type') == 'tool_result':
            ...
            out.append(f"--- Step {step_idx} [FAILED] ---")
...
def planner_node(state: PevState) -> Dict:
    ...
    # TRUNCATION: If Wiki is too long, we only show tool names to save tokens.
    tool_wiki_section = ""
    if state.tools_wiki:
        wiki_text = state.tools_wiki
        if len(wiki_text) > 5000:
             wiki_text = wiki_text[:5000] + "... [WIKI TRUNCATED]"
        tool_wiki_section = f"\nTECHNICAL TOOL WIKI (Tools available for the Executor):\n{wiki_text}\n"
...
"""
    ...
    # SLIDING WINDOW: Only show the original goal and the last 5 turns of conversation
    user_msgs = []
    if state.user_conversation:
        user_msgs.append(SystemMessage(content=state.user_conversation[0]['content']))
    
    # Recent context
    for turn in state.user_conversation[-5:]:
        if turn['role'] != 'system' and turn != state.user_conversation[0]:
