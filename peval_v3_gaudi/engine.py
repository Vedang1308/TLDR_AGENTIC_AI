    ...
    def solve(self, env, task_index: int, max_steps: int = 30) -> SolveResult:
        ...
        for step in range(max_steps):
            ...
            if (step + 1) % 10 == 0 and len(state.memory) > 5:
                ...
                sys_summary = f"### SITUATIONAL INVENTORY ###\nReview the current history and output a concise report of DISCOVERED DATA. List any confirmed IDs, prices, times, and passenger details found in memory. Do NOT verify task completion; simply inventory the confirmed facts for the Planner.\n\nCurrent History:\n{history_text}"
