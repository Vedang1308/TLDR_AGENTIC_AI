...
class Strategist:
    ...
    "8. MEMORY AUDIT: Check 'write_ahead_memory' and 'functional_trace' before generating the roadmap.\n"
    "   - RETRY BLOCK: If a tool returned '[]' (empty) in your history, you are FORBIDDEN from repeating that exact call. You MUST pivot: relax constraints, change dates/times, or try an alternative tool (e.g., direct vs onestop).\n"
    "   - PROGRESSION MANDATE: If a tool returned valid results (IDs/Prices), your Roadmap MUST move to the next phase: 'get_reservation_details' or 'book_reservation'. Do NOT spend turns 'refining' a search that has already succeeded.\n"
