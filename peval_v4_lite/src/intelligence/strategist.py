...
class Strategist:
    ...
    def __init__(self, tools_info: list):
        ...
        self.system_prompt = (
            ...
            "   - HALLUCINATION BLOCK: DO NOT invent tool names. You are forbidden from using names like 'search_onestop_flight' or 'search_direct_flight' unless they appear EXACTLY in the list below.\n"
            "   - If your goal is to 'Find/Retrieve a record', identify the official tool that queries the catalog.\n"
            "   - If a discovery tool returns a list of candidate IDs/summaries, you MUST call the corresponding 'get_details' tool for each candidate to verify specific constraints (times, prices, attributes) before committing to an action.\n"
            "   - STRICT LIST COMPLIANCE: If the required tool is not in the list, you MUST acknowledge the data gap and pivot (e.g., call 'list_all_product_types').\n"
    ...
    "8. WORLD SNAPSHOT (VARIABLE STORAGE): Treat the 'world_snapshot' as your primary variable file. \n"
    "   - FORBIDDEN ASKS: You are strictly forbidden from asking the user for information that is already present as a variable in the Snapshot. \n"
    "   - RETRY BLOCK: If a tool returned '[]' in history, relax constraints or try an alternative tool. Do not repeat identical failed calls.\n"
    "   - PROGRESSION MANDATE: If a successful ID was discovered, your roadmap MUST move to the next phase (get_details or book).\n"
    ...
    "11. CONCISION MANDATE: Your 'refined_tactical_plan' MUST be a direct, imperative instruction (e.g., 'Search for flights from JFK to SEA'). Never include conversational justification like 'I will now...' or 'Because we need...'. Keep it strictly technical.\n"
    "12. DISCOVERY-TO-ACTION GATE: If 'available_options' (Blackboard SSO) is NOT empty, you ARE FORBIDDEN from performing any more 'search' tool calls. You MUST transition to 'get_reservation_details', 'get_user_details', or 'respond' to present the discovered options.\n"
    "13. INERTIA BREAKER: Before drafting a search roadmap, scan the 'FUNCTIONAL TRACE' for successful matches. If the trace shows a previous successful search for the current constraint, use those results instead of searching again.\n\n"
            "AVAILABLE TOOLS:\n"
