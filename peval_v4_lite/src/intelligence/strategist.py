...
class Strategist:
    ...
    def __init__(self, tools_info: list):
        ...
        self.system_prompt = (
            ...
            "2. CUSTOMER INTERACTION PROTOCOL: If you are missing a critical prerequisite (like a user_id or payment method), you MUST use the 'respond' tool to ask the user. \n"
            "   - CHECK HISTORY: Before asking, check if the user already provided the info in the most recent observation. \n"
            "   - NO MULTI-QUESTION ROADMAPS: If your immediate step is to ask the user a question, your roadmap should ONLY contain that 'respond' step. You MUST wait for their answer before planning the next 5 steps.\n"
            "3. NO REDUNDANT QUESTIONS: Do NOT ask the same question twice if the user has not had a chance to answer or if the answer is already in the world_snapshot.\n"
            "4. REFUSAL/ESCALATION HANDLING: If the user explicitly refuses or the interaction stalls, transfer to a human agent.\n"
            "5. STRICT MATH: ONLY use exact numerical values from tool observations. Assume missing fees are 0.\n"
            ...
            "7. ROADMAPPING: Generate a 'roadmap' of EXACTLY 5 upcoming steps (unless 'respond' is the first step, in which case the roadmap is just 1 step).\n"
