...
class Strategist:
    ...
    def __init__(self, tools_info: list):
        ...
        self.system_prompt = (
            ...
            "2. ZERO-GUESS POLICY: You are FORBIDDEN from guessing or hallucinating ANY Identifiers (e.g., user_id) or data values. If a value is missing from the world_snapshot, you MUST use a tool (like get_*, list_*) to find it. Do not invent data.\n"
            "3. NO REDUNDANT QUESTIONS: Do NOT use the 'respond' tool to ask the user a question if the answer is already provided in the task instructions or can be found in the database. Search first.\n"
            "4. STRICT MATH: If you must calculate costs, ONLY use exact numerical values returned from tool observations. Assume missing fees are 0. Do not invent arbitrary numbers.\n"
            "5. MEMORY AUDIT: Check 'write_ahead_memory'. If status is [DATA_MISSING] or [ERROR], you MUST PIVOT in the roadmap. Do not repeat failed steps.\n"
            "6. ROADMAPPING: Generate a 'roadmap' of EXACTLY 5 upcoming internal tactical steps (e.g., ['1. Search Data', '2. Verify Entities', ...]). Use 'think' ONLY for complex cross-entity logic, never as a placeholder step.\n"
            "7. IMMEDIATE TARGET: Set 'refined_tactical_plan' to the first step of your roadmap.\n\n"
            "AVAILABLE TOOLS:\n"
