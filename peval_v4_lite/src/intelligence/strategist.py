...
class Strategist:
    ...
    def __init__(self, tools_info: list):
        ...
        self.system_prompt = (
            ...
            "2. CUSTOMER INTERACTION PROTOCOL: You are serving a human user. If you are missing a critical prerequisite (like a user_id, payment method, or exact name) to execute a database search or booking, you are STRICTLY FORBIDDEN from guessing it. You MUST use the 'respond' tool to politely ask the user for it.\n"
            "3. NO REDUNDANT QUESTIONS: Check the 'Context' first. If the user already provided the info in the task instructions or a previous message, use it. Do NOT ask again.\n"
            "4. REFUSAL/ESCALATION HANDLING: If you repeatedly ask the user for required information and they explicitly refuse to provide it or the interaction stalls, you MUST escalate by plotting a roadmap step to transfer to a human agent.\n"
            "5. STRICT MATH: If you must calculate costs, ONLY use exact numerical values returned from tool observations. Assume missing fees are 0. Do not invent arbitrary numbers.\n"
            "6. MEMORY AUDIT: Check 'write_ahead_memory'. If status is [DATA_MISSING] or [ERROR], you MUST PIVOT in the roadmap. Do not repeat failed steps.\n"
            "7. ROADMAPPING: Generate a 'roadmap' of EXACTLY 5 upcoming internal tactical steps (e.g., ['1. Verify Identity', '2. Search Flights', ...]). Use 'think' ONLY for complex cross-entity logic, never as a placeholder step.\n"
            "8. IMMEDIATE TARGET: Set 'refined_tactical_plan' to the first step of your roadmap.\n\n"
            "AVAILABLE TOOLS:\n"
